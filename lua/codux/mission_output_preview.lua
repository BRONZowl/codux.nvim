...
function M.output_preview_running(self)
  local job_id = self.state.mission_dashboard.output_job
...
function M.close_output_preview(self)
  ...
  local preview = self.state.mission_dashboard.output_preview
...
function M.start_output_preview(self, entry, opts)
  ...
  if type(entry) == "table" and type(self.reconcile_workspace_entry) == "function" then
    ...
    elseif reconcile_error then
      ...
      self.state.mission_dashboard.output_blocked_key = self.state.mission_dashboard.output_key
      return self:render_output_status(entry, reconcile_error)
    end
  end

  self:close_output_preview()
  self.state.mission_dashboard.output_generation = (tonumber(self.state.mission_dashboard.output_generation) or 0) + 1
  local generation = self.state.mission_dashboard.output_generation
  self.state.mission_dashboard.output_entry = entry
  self.state.mission_dashboard.output_key = self:output_entry_key(entry)
  ...
  local preview_key = self.state.mission_dashboard.output_key
  local preview_buf = self.state.mission_dashboard.output_buf
  local term_ok, term_error = pcall(vim.api.nvim_buf_call, self.state.mission_dashboard.output_buf, function()
    return self.termopen(command, {
      on_exit = function(exited_job_id, code)
        if
          self.state.mission_dashboard.output_job == exited_job_id
          and self.state.mission_dashboard.output_generation == generation
          and self.state.mission_dashboard.output_key == preview_key
        then
          self.state.mission_dashboard.output_job = nil
          local active_entry = self.state.mission_dashboard.output_entry
          local active_key = self.state.mission_dashboard.output_key
          local active_preview = self.state.mission_dashboard.output_preview
          self.state.mission_dashboard.output_preview = nil
          ...
          if was_control then
            ...
            self.state.mission_dashboard.output_blocked_key = active_key
  ...
  self.state.mission_dashboard.output_job = term_error
  self.state.mission_dashboard.output_preview = preview
